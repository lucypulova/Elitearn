...
export default function Catalog({ onGoMyCourses }: CatalogProps) {
  ...
  const [recMode, setRecMode] = useState<"for_you" | "generic">("generic");
  ...
  useEffect(() => {
    ...
    if (!qq) return;
    const handle = setTimeout(() => {
      api.post("/search/log", { query: qq, context: "catalog" }).catch(() => void 0);
    }, 650);
    return () => clearTimeout(handle);
  }, [q]);

  const fetchRecommendations = async (seedCourseId: number | null, mode: "for_you" | "generic") => {
    try {
      setRecError("");
      setRecLoading(true);
      ...
      if (mode === "for_you" && user) {
        const res = await api.get("/me/recommendations", { params: { limit } });
  ...
  useEffect(() => {
    fetchRecommendations(null, user ? "for_you" : "generic");
  }, []);

  useEffect(() => {
    if (recSeedCourseId == null) fetchRecommendations(null, user ? "for_you" : "generic");
  ...
  const addToCart = async (
    ...
    if (!user) {
      ...
      fetchRecommendations(courseId, "generic");
      return;
    }
    ...
    try {
      ...
      fetchRecommendations(null, "for_you");
    } catch (e: any) {
        ...
        <div className="elite-count-row">
          <strong>
            ...
            {recSeedCourseTitle ? (
              ...
              <span className="elite-muted" style={{ marginLeft: 10 }}>
                ({recMode === "for_you" && user ? "препоръчани за вас" : user ? "популярни" : "последно добавени"})
              </span>
            )}
          </strong>

          <div style={{ display: "flex", gap: 8 }}>
            {user && !recSeedCourseTitle ? (
              <button
                className={`elite-btn sm ${recMode === "for_you" ? "primary" : "ghost"}`}
                type="button"
                onClick={() => fetchRecommendations(null, "for_you")}
                title="Персонализирано спрямо покупки и кошница"
              >
                За вас
              </button>
            ) : null}
            ...
            {user && !recSeedCourseTitle ? (
              <button
                className={`elite-btn sm ${recMode === "generic" ? "primary" : "ghost"}`}
                type="button"
                onClick={() => fetchRecommendations(null, "generic")}
                title="Популярни препоръки"
              >
                Популярни
              </button>
            ) : null}
