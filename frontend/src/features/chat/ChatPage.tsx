  ...
  return (
    <div className="elite-wrap">
      ...
      <div className={styles.panel}>
        ...
        <div className={styles.list}>
          ...
          {conversations.map((c) => {
            ...
            const avatar = normalizeAvatar(c.other_user_avatar_url);
            return (
              <button
                key={c.id}
              ...
              >
                <div className={styles.itemRow}>
                  <div
                    ...
                    style={avatar ? { cursor: "zoom-in" } : undefined}
                    aria-label={avatar ? "Отвори профилната снимка" : ""}
                  >
                    {avatar ? <img src={avatar} alt="avatar" /> : <div className={styles.avatarFallback}>{c.other_user_email.slice(0, 1).toUpperCase()}</div>}
                  </div>
      ...
      <div className={styles.panel}>
        ...
        <div className={styles.messages} ref={scrollerRef}>
          {!activeConvId ? (
            ...
            messages.map((m) => {
              ...
              const senderAvatar = normalizeAvatar(m.sender_avatar_url);
              const senderLabel = (m.sender_email || "?").slice(0, 1).toUpperCase();
              return (
                <div key={m.id} className={`${styles.msgRow} ${mine ? styles.mine : styles.theirs}`}>
                  {!mine ? (
                    <div
                      className={styles.avatarSm}
                      role={senderAvatar ? "button" : undefined}
                      tabIndex={senderAvatar ? 0 : -1}
                      onClick={() => {
                        if (senderAvatar) setAvatarPreviewUrl(senderAvatar);
                      }}
                      onKeyDown={(e) => {
                        if (!senderAvatar) return;
                        if (e.key === "Enter" || e.key === " ") {
                          e.preventDefault();
                          setAvatarPreviewUrl(senderAvatar);
                        }
                      }}
                      style={senderAvatar ? { cursor: "zoom-in" } : undefined}
                      aria-label={senderAvatar ? "Отвори профилната снимка" : ""}
                    >
                      {senderAvatar ? <img src={senderAvatar} alt="avatar" /> : <div className={styles.avatarFallback}>{senderLabel}</div>}
                    </div>
                  ) : null}
        ...
        {avatarPreviewUrl ? (
          <div className="elite-modalOverlay" onClick={() => setAvatarPreviewUrl(null)} role="dialog" aria-modal="true">
            <div className="elite-modal" onClick={(e) => e.stopPropagation()}>
              <div className="elite-modalHead">
                ...
                <button className="elite-btn sm" onClick={() => setAvatarPreviewUrl(null)}>
                  Затвори
                </button>
              </div>
              <div className="elite-modalBody">
                <img className="elite-modalImg" src={avatarPreviewUrl} alt="Avatar" />
              </div>
            </div>
          </div>
        ) : null}
